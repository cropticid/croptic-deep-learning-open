...
model = dict(
    ...
    query_head=dict(
        ...
        transformer=dict(
            type='CoDinoUAVDetrTransformer',
            with_pos_coord=True,
            with_coord_feat=False,
            num_co_heads=2,
            num_feature_levels=4,
            encoder=dict(
                type='UAVdetrTransformerEncoder',
                num_layers=1,
                with_cp=1, # number of layers that use checkpoint
                use_encoder_idx=[3],
                feat_strides=[8, 16, 32, 64],
                in_channels_ccff=[256, 256, 256, 256],
                hidden_dim=256,
                transformerlayers=dict(
                    type='BaseTransformerLayer',
                    attn_cfgs=dict(
                        type='MultiScaleDeformableAttention', embed_dims=256, num_levels=1, dropout=0.0),
