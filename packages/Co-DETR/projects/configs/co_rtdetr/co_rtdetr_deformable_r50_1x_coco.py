...
model = dict(
    ...
    query_head=dict(
        ...
        num_query=300,
        num_classes=5,
        in_channels=2048, # tidak digunakan, hanya untuk single feature bukan multi-scale
        sync_cls_avg_factor=True,
        with_box_refine=True,
        as_two_stage=True,
        mixed_selection=True,
        transformer=dict(
            type='CoRTDetrDeformableTransformer',
            num_co_heads=2,
            encoder=dict(
                ...
                in_channels_ccff=[256, 256, 256],
                hidden_dim=256,
                transformerlayers=dict(
                    type='BaseTransformerLayer',
                    attn_cfgs=dict(
                        type='MultiScaleDeformableAttention', embed_dims=256, dropout=0.0, num_levels=1),
                    ffn_cfgs=dict(
                        embed_dims=256),
